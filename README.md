...
Every statement is checked against itself: the parser sums the transaction lines and compares them with the totals the bank printed. The verification block in the report shows both numbers, so a layout the parser does not fully understand is visible, never silent. Layouts verified to the cent so far (`scripts/parser_check.py` over public court-filing exhibits and the test statements; 274 statements to the cent as of September 19, 2026 afternoon, over about 4,200 court-filing PDFs, 47 of them with funder activity, 50 more documents recognized as not bank statements):
