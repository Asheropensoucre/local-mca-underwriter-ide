...
const GB: f64 = 1e9;
/// RAM left for the desktop, the browser and everything else when the engine is at
/// its largest. Below this the machine swaps and, on shared-memory GPUs, freezes.
pub const HEADROOM_BYTES: u64 = 2_500_000_000;
/// The watchdog stops the engine when available memory falls under this.
pub const WATCHDOG_FLOOR_BYTES: u64 = 900_000_000;
...
const IMAGE_BYTES_PER_SLOT: u64 = 900_000_000;
...
/// Size the engine for the memory available now. Tries the full presets first, then
/// smaller ones; the smallest that fits wins, and if nothing fits the plan says so.
pub fn plan(ocr: &ModelSpec, underwriter: &ModelSpec) -> MemoryPlan {
    ...
    for &(octx, opar, uctx, max) in &candidates {
        ...
        if peak + HEADROOM_BYTES <= available {
            chosen = Some((octx, opar, uctx, max, peak));
            break;
        }
    }
    let (octx, opar, uctx, max, peak, fits) = match chosen {
        Some((a, b, c, d, p)) => (a, b, c, d, p, true),
        None => {
    ...
    let message = if fits {
        format!(
            "{:.1} GB free of {:.0} GB; the engine will use up to {:.1} GB ({} model{} loaded at a time, {} OCR page{} at a time)",
            available as f64 / GB, total as f64 / GB, peak as f64 / GB, max, if max == 1 { "" } else { "s" }, opar, if opar == 1 { "" } else { "s" }
        )
    } else {
        format!(
            "Not enough free memory: {:.1} GB free of {:.0} GB, the engine needs about {:.1} GB plus {:.1} GB left for the rest of the system. Close other programs and try again.",
            available as f64 / GB, total as f64 / GB, peak as f64 / GB, HEADROOM_BYTES as f64 / GB
        )
    };
    MemoryPlan { total_bytes: total, available_bytes: available, models_max: max, ocr_ctx: octx, ocr_parallel: opar, underwriter_ctx: uctx, peak_bytes: peak, fits, message }
}
...
pub fn check_before_job(ocr_parallel: u32) -> Result<(), String> {
    let (total, available) = available_bytes();
    let need = IMAGE_BYTES_PER_SLOT * ocr_parallel as u64 + HEADROOM_BYTES / 2;
    if available < need {
        return Err(format!(
