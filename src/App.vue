        ...
        <div class="flex-1 p-5 overflow-auto">
          ...
          <div v-if="activeTab === 'underwrite'" class="space-y-4 h-full flex flex-col">
            ...
            <div>
              ...
              <!-- Test Result Display -->
              <div v-if="testThoughts || testResult" class="mt-3 p-3 bg-slate-800/50 border border-slate-700 rounded-lg">
                <div v-if="testThoughts" class="mb-2">
                  <div class="flex items-center gap-1 mb-1">
                    <svg class="w-3 h-3 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                    </svg>
                    <span class="text-xs font-medium text-purple-300">AI Thoughts:</span>
                  </div>
                  <p class="text-xs text-gray-400 italic font-mono whitespace-pre-wrap bg-slate-900/50 p-2 rounded">{{ testThoughts }}</p>
                </div>
                <div v-if="testResult" class="text-xs text-gray-300">
                  <span class="font-medium">Response:</span> {{ testResult }}
                </div>
              </div>
            </div>
